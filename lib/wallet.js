...
module.exports = class MongoDBWallet extends Wallet {
  ...
  async remove(name) {
    ...
    return await this.wallet.remove({ name: name }).exec()
  }
   ...
   */
  async listNames() {
    let result = await this.wallet.find({}, {'_id': false, 'name': true}).exec()
    const list = []
    result.forEach((elt) => {
      list.push(elt.name)
    })
  ...
  async contains(name) {
    ...
    let result = await this.wallet.findOne({ name: name }).exec()
    return (result !== null)
  }
  ...
  async get(name) {
    ...
    let card = await this.wallet.findOne({ name: name }).exec()
    if (!card) {
